      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        {inputFields.map(({ id, label, type }) => (
          <div key={id} className="mb-4 relative">
            <label
              htmlFor={id}
              className="block text-gray-700 text-lg dark:text-gray-300 font-bold mb-2"
            >
              {label}
            </label>
            <div className="relative">
              <input
                ...
                name={id}
                {...register(id)}
                className={`w-full px-4 py-4 border-4 border-gray-300 dark:text-white dark:bg-gray-800 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-500 ${
