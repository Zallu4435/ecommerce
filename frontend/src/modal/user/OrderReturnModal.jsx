...
const ReturnConfirmationModal = ({
  ...
  const displayTitle = title || "Return Item"
  const displayMessage = message || `Are you sure you want to return ${productName || 'this item'}? This will initiate a return request.`
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center backdrop-blur-sm justify-center p-4 sm:p-6 z-50 animate-fadeIn">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 lg:p-8 max-w-lg w-full shadow-2xl transform transition-transform duration-300 scale-95 hover:scale-100 border border-gray-100 dark:border-gray-700">
        <h2 className="text-xl sm:text-2xl lg:text-3xl font-extrabold mb-4 sm:mb-6 text-amber-700 dark:text-gray-100 leading-tight">
          {displayTitle}
        </h2>
        ...
        <div className="mb-6">
          ...
          <textarea
            className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-gray-900 dark:text-gray-100 dark:border-gray-600 text-sm sm:text-base transition-all resize-none"
            rows="4"
            value={reason}
