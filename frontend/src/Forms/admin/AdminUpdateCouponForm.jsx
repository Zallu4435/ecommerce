...
import { useGetCouponByIdQuery } from '../../redux/apiSliceFeatures/couponApiSlice';
...
const AdminCouponUpdateForm = () => {
  ...
  const { data, error, isLoading } = useGetCouponByIdQuery(id); 
  ...
  useEffect(() => {
    ...
    if (data) {
      setValue('couponCode', data?.coupon?.CoupenCode || '');
      setValue('discount', data?.coupon?.Discount?.$numberDecimal || '');
      setValue('expiry', formatDate(data?.coupon?.Expiry) || '');
      setValue('maxDiscount', data?.coupon?.MaxDiscount?.$numberDecimal || '');
      setValue('minPurchase', data?.coupon?.MinPurchase?.$numberDecimal || '');
    }
  }, [data, setValue]);
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      navigate(-1); // Redirect to the coupons page
    } catch (err) {
